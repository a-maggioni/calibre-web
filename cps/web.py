    ...
    if request.method == "POST":
        ...
        if config.config_use_ldap and user:
            ...
            try:
                ...
                ipAdress = request.headers.get('X-Forwarded-For', request.remote_addr)
                app.logger.info('LDAP Login failed for user "' + form['username'] + '" IP-adress: ' + ipAdress)
                flash(_(u"Generic error with LDAP"), category="error")
    ...
    if request.method == "POST":
        ...
        if "config_use_ldap" in to_save and to_save["config_use_ldap"] == "on":
            if not "config_ldap_provider_url" in to_save or not "config_ldap_dn" in to_save:
                ...
                content.config_ldap_enable_tls = 1 if "config_ldap_enable_tls" in to_save and to_save["config_ldap_enable_tls"] == "on" else 0
                content.config_ldap_ignore_certificate = 1 if "config_ldap_ignore_certificate" in to_save and to_save["config_ldap_ignore_certificate"] == "on" else 0
                db_change = True
